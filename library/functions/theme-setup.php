...
if( !function_exists('sp_theme_setup') )
{
	function sp_theme_setup(){
		...
		add_image_size('thumb-medium', 300, 200, true ); // base size 1240x768
...
?>
	...
	<?php if ( ot_get_option('header-image') != '' ) : ?>
		#header { background-image:url(<?php echo ot_get_option("header-image"); ?>); }
	<?php endif; ?>
	</style>
...
if (!function_exists('sp_footer_nav_fallback')) {
	...
	function sp_footer_nav_fallback() {
		...
		$menu_html = '<ul class="footer-nav clear">';
...
function sp_custom_login_logo() {
	...
	if ($custom_logo) {	
	    $out .='.login h1 a { background-image:url('.ot_get_option('custom-logo').') !important; height: 100px!important; width: 100%!important; background-size: 80%!important;}';
	} else {
...
function sp_head_meta() { ?>
	...
	<?php if ( ot_get_option('responsive') != 'off' ) { ?><meta content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0" name="viewport">
	<?php } ?>
